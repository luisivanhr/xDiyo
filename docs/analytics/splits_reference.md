...
create_split_plan(dataset, splitter, **split_options) -> SplitPlan
```
...
splitter's `folds` signature. Plans contain no `model_selector` or `refit_policy`
fields, and the factory has no such named parameters. Supplying those options to
a built-in splitter raises `TypeError` as unsupported split options. Fitting
configuration belongs to the future training runner, where both options are
intended to default to `None`. Fold reporting is independently usable.

The factory creates no inner folds, features, fitted state, selector run or model
fit. Custom splitter objects can use that same interface
...
| `unit` | `rounds`, `seasons`, `league_seasons` or `kickoffs`; controls default calendar/block keys. Seasons pool shared source-season labels across leagues; league-seasons retain separate league folds. |
| `gap` | Nonnegative integer, default 0; counts `gap_unit` blocks. |
| `gap_unit` | `None` inherits `unit`. An equal-scale gap skips whole window blocks between training and testing. A finer gap removes leading blocks inside the nominal test span without extending its end. Round gaps count separately per competition, using numeric round order within each native season; kickoff gaps use unique times pooled in the current calendar. Coarser units are rejected. |
...
| `calendar_by` | `None` means `('competition_id',)` for rounds/league-seasons and `()` for seasons/kickoffs. Empty tuple pools. Other names group independent calendars. |
| `block_by` | `None` means `('season_id','round')` for rounds, `('source_season',)` for seasons, `('season_id',)` for league-seasons, or `('kickoff_at',)` for kickoffs. Explicit nonempty names replace that identity. Include stage when rounds repeat across stages. |
...
A finer round gap that consumes every round for any included league raises.
A finer kickoff gap that consumes the entire test horizon also raises.
Scoring rows intersect the test rows retained after the gap. Use
`unit='league_seasons'` to migrate code needing the former separate-league
default of `unit='seasons'`.
...
| `gap`, `gap_unit`, `gap_rows` | Requested gap count, resolved gap unit and original positions excluded by the gap. For a finer gap, `test_blocks` still describes the nominal horizon. |
