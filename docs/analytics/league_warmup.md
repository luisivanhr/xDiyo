...
for focal team \(u\). For each statistic column, define the finite-observation set

\[
I_{t,u}=\{i\in W_t\setminus E_u:x_i\text{ is finite}\},
\qquad n=|I_{t,u}|.
\]
...
During active warm-up, features use their seeded or blended mean and
\(s=\sqrt{v}\). At zero warm-up weight, or without a finite mean prior, the
ordinary rolling calculation and its configured `ddof` apply. The Z-score is
...
Writing \(\lambda=\texttt{league\_weight}\), with \(\lambda\in[0,1]\),
the initial team mean is
...
Here \(N_L>0\) counts finite observations in the matching league population.
The variance is centered on that league's own mean, even if \(m_0\) is
team-specific. With no finite league observations, \(v_0\) is missing.
...
observation, and \(\alpha\in(0,1]\) the `alpha` parameter. Each eligible current-season
...
If starting from an ordinary variance \(s_d^2\) with `ddof` \(d\), the equivalent conversion
...
With \(c=\texttt{phi\_scale}\), \(c_M=\texttt{movement\_phi\_scale}\),
\(s=\texttt{shrinkage}\), and a nonempty selected rating cohort \(C\),
where selection means ranking and taking the required top/bottom members,
...
\]
\[
\mu_{\mathrm{new}}=\begin{cases}
(1-s)\mu_{\mathrm{old}}+s\mu_C,
 &\text{mover with a nonempty selected cohort},\\
\mu_{\mathrm{old}},&\text{otherwise},
\end{cases}
\]
\[
...
\qquad \sigma_{\mathrm{new}}=\sigma_{\mathrm{old}}.
\]

The multipliers satisfy \(c,c_M\geq1\), and \(s\in[0,1]\).
For every non-mover or an empty selected mover cohort, \(\mu\) stays unchanged.
