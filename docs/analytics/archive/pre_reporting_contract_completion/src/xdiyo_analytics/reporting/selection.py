...
class TopKCorrelationSelector:
    ...
    def run(self, context):
        ...
        if self.across_folds:
            return self._across(context, features, targets)
        ...
        chosen = tuple(ranking.index[ranking.pooled_magnitude.notna()][:self.k])
        ...
        if len(chosen) < self.k:
            notes.append(f"Requested {self.k}; only {len(chosen)} features had defined scores.")
        ...
        return StudyResult(
            title=f"Top {self.k} · {self.method}",
    ...
    def _across(self, context, features, targets):
        ...
        if isinstance(fold_k, bool) or not isinstance(fold_k, (int, np.integer)) or fold_k < 1:
            raise ValueError("fold_k must be a positive integer or None.")
        source_results = context.fold_results.get(self.source, {}) if self.source is not None else {}
        if self.source is not None and set(source_results) != set(context.fold_rows):
            raise ValueError("Consensus source needs a matching correlation result for every selected fold.")
        fold_tables, coefficient_tables = [], []
        for fold_id, rows in context.fold_rows.items():
            if not set(rows) <= set(context.row_positions):
                raise ValueError("Consensus context must contain the selected fold partition rows.")
            child = replace(context, X=context.X.loc[rows].copy(), y=context.y.loc[rows].copy(),
                            ...
                            previous_results={} if self.source is None else {self.source: source_results[fold_id]},
                            fold_rows={}, fold_results={})
            local = replace(self, type="per_fold", k=fold_k, across_folds=False, fold_k=None).run(child)
            table = local.tables["ranking"].copy()
            table.insert(0, "fold_id", fold_id)
            fold_tables.append(table)
            coefficients = local.tables["coefficients"].copy()
        ...
        grouped = per_fold.groupby("feature", sort=False)
        ranking = pd.DataFrame(index=pd.Index(features, name="feature"))
        ranking["votes"] = grouped.selected.sum().reindex(features).fillna(0).astype(int)
        ranking["mean_magnitude"] = grouped.pooled_magnitude.mean().reindex(features)
        ranking["scored_folds"] = grouped.pooled_magnitude.count().reindex(features)
        ranking["vote_fraction"] = ranking.votes / len(context.fold_rows)
        ranking = ranking.sort_values(["votes", "mean_magnitude"], ascending=False, kind="stable", na_position="last")
        chosen = tuple(ranking.index[(ranking.votes > 0) & ranking.mean_magnitude.notna()][:self.k])
        ranking["selected"] = ranking.index.isin(chosen)
        ranking = ranking.reset_index()
        return StudyResult(
            title=f"Top {self.k} · {self.method} fold consensus",
            ...
            notes=[f"Each of {len(context.fold_rows)} folds nominates up to {fold_k} features.",
                   "Ordered by nomination count, then mean absolute pooled magnitude, then input feature order.",
                   f"Selected {len(chosen)} nominated features; requested {self.k}.",
