...
Initial configurable defaults: rating 1500, RD 350, volatility .06, tau .5; one event-driven period per match, with equal-release-time events batched against shared pre-batch states. Both teams update from preceding states, once per match. No automatic calendar inactivity inflation or season reset; competition scope is default, scope=() follows teams across leagues and adding season_id resets per season explicitly. These are initial implementation defaults, not claims of optimal football settings.
...
Background verification/docs/notebook work is delegated, including the official worked example, independent source/replay comparisons and complete regression suite. Implementation is complete; verification remains pending until recorded. No source exports, legacy rating utility, scraper or D-Tale session were modified.
