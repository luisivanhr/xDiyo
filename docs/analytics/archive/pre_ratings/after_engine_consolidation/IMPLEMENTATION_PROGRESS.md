...
## General ratings and Glicko features — implemented; verification pending
...
Background verification/docs/notebook work is delegated, including integration/real-data checks and complete regression suite on the consolidated source. Numerical parity is verified; broader rating integration remains pending until recorded. No source exports, legacy rating utility, scraper or D-Tale session were modified.
...
3. Use the executed 8-cell `02_feature_quickstart.ipynb` and verified feature API to select the pilot recipe. Default kickoff cutoffs and the earlier-finished-kickoff availability proxy are documented; explicit availability remains fully optional. Preserve the exact source list and selection records in future experiment configuration. Targets, rating-state integration, CV and model fitting remain subsequent work.
