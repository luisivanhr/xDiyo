...
## Multi-season loading — implemented; verification and experiment import pending
...
The experiment notebook should now import all 39 available league-season publications for 22_23/23_24/24_25, selecting matches, statistics and pregame for initial feature work. Shots remain independently selectable. Run the actual import and summarize counts in the background task, without adding a long walkthrough.

**Coverage policy:** `coverage` is optional and is not selected for this experiment or required by either loader. A future publication can omit its table descriptor/file and the catalogue follows the remaining manifest. Explicitly requesting an absent coverage table should still raise a clear unavailable-table error. Do not delete collected files now or change historical pinned manifests in place; the user's removal plan concerns future exports/library publication. Background tests should demonstrate loading/inspection of an export with no coverage table.

## Simplification rewrite — implemented and verified
...
1. Use the short loading notebook and the two everyday calls, `inspect_season` and `load_season`, to choose inputs for the first experiment. The detailed checkpoint history below is archived-era context.
