...
**Confirmed discussion order:** review league population/window/LOO semantics, then optional season-transition policies, before returning to the shared dataset contract and reusable orchestration. These extensions are **proposed, not implemented**. The existing **no-warm-up, available-history default remains unchanged**. This update authorizes discussion only, with no code, tests, notebooks or pilot configuration requested. (D)

- **League population and timing — pending choices:** distinguish one team's contribution from one match total, and define window population, weighting and update schedule. Completed-round mode should freeze the league reference population at the target round's prediction anchor for all fixtures in that round; kickoff mode may evolve as eligible results arrive. Postponed, staged and incomplete rounds need explicit treatment. Current eligibility helpers require `team_id`; league histories need explicit support, not just different grouping columns.
- **LOO — proposed calculation:** choose the eligible population, exclude the focal team, then finalize aggregates. Excluding only its team rows differs from excluding every fixture involving it. Reuse league/team sufficient summaries where valid, but recompute moments after exclusion; never subtract or average standard deviations. Pooled-observation variance and variance of round means are different pending definitions. A frozen round population still produces team-specific LOO baselines. A league-relative team Z-score needs a known historical team numerator in units matching its baseline. LOO excludes observations from a feature population; it differs from LOCO feature-importance refitting.
...
- **Priors and state transfer — pending choices:** destination top/bottom cohorts, mean/median, cohort sizes and ranking criteria must be explicit. Freeze their snapshots before the transition and exclude the moving team. Current ratings are competition-scoped, so retrieving a previous-league state and deciding how league rating scales are calibrated/transferred are additional explicit operations; membership evidence alone does not settle them.
- **Simple feature warm starts — proposed first approach:** optionally seed a mean EMA from previous-season and/or destination-league means, with explicit blend, decay and time unit. Continuing that EMA differs from temporarily warming a fixed rolling window; any handoff needs a defined policy. Std/variance/Z-score warm starts also need prior spread and weight, not only a prior mean. James-Stein and more elaborate cross-league borrowing remain deferred beyond this simple approach.

The discussion order is confirmed; the calculation choices and suggested implementation sequence still need review. The earlier orchestration-first instruction is superseded only as the immediate next step: reusable orchestration still precedes configuring the corner experiment. Historical proposals below remain reference material. (D)
...
| **Season and league transitions** | No-warm-up/available-history remains the default. Optional shared transition/prior helpers with feature-specific adjustments applied once are proposed for discussion before orchestration. Reuse ID-based movement evidence with new exports and freeze destination priors before transition, excluding the moving team. Exact adjustments, state transfer and cross-league rating calibration remain open. Boundaries do not automatically reset models or CV. |
| **Optional prior/exponential strategy** | Begin the design with an optional seeded EMA for means, using explicit previous-season/destination priors, blend and decay; this does not change the default. EMA continuation and a handoff to fixed rolling windows need separate policies. Prior spread/weight is additionally required for std/variance/Z-score. The original `LinearWarmStart` schedule remains illustrative; James-Stein is deferred. Glicko follows its own initialization/update rules. |
...
| **Historical grouping and alignment** | The first evaluator defaults to team+competition across venues/seasons; custom grouping is supported and H2H adds ordered opponent identity. Target identity and cutoff ties are excluded. League histories/LOO are the next design discussion: observation/exclusion units, frozen-round versus evolving-kickoff populations and variance definitions remain pending, as detailed above. Match-wide assembly remains later work. |
