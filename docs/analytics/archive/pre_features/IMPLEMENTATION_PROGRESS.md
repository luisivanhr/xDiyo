...
## Feature expressions and shared history helpers — implemented; verification pending
...
The existing background task owns meaningful behavior/regression tests, representative real-data checks, compact notebook usage and documentation. Implementation is complete; verification remains pending until recorded there. No source exports, running D-Tale or scraper processes were changed.
...
3. Define the first feature inputs and historical availability/cutoff policies against the observed team rows, retaining the exact source list and existing per-publication selection records with future experiment configuration. Kickoff order alone does not establish eligible information. Lags, rolling features and model fitting remain future work.
