...
**Latest priority — finish reusable orchestration before designing the pilot:** the user explicitly wants the remaining orchestration layers established before configuring the corner experiment. Next work is the shared feature/target/metadata/layout contract and generic execution flow, followed by reusable labels, assembly, temporal splits, model adapters and both pre-training and post-training reporter stages. Do not select a pilot feature recipe, tune models or launch fitting yet. The previously selected leagues/seasons and eventual pilot remain context, not authorization to skip these layers.
...
Evidence: [features_check.json](docs/analytics/features_check.json). Immediate next work remains selecting the pilot's exact feature recipe and preparing targets, rating-state integration, CV and fitting under their separately agreed contracts. No model, prior, warm-up, raw-source join or source export change was introduced in this batch. Running D-Tale and scraper processes were preserved.
...
1. Use the executed 12-cell notebook with `load_seasons`, `select_stats` and `build_team_history` for the selected 13-league, three-season population. `list_stat_bundles` discovers bundles; `inspect_season` and `load_season` remain useful for one publication. The detailed checkpoint history below is historical context.
2. Choose the first model's statistic bundles, exact measures and any category overrides, including periods/groups/keys and optional context before joins.
3. Use the executed 8-cell feature and ratings notebooks (`02_feature_quickstart.ipynb`, `03_ratings_quickstart.ipynb`) to select the pilot recipe. Default kickoff cutoffs and the earlier-kickoff availability proxy are documented; explicit availability remains fully optional. Result/statistic rating state and saved-run feature integration are verified. Preserve the exact source list and records in future experiment configuration. Targets, CV/model fitting and future graph-state producers remain subsequent work.
