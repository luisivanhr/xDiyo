...
league round-window length. Std/Z require `n > ddof`, default `ddof=1`.
...
Let `m,v` be the seeded mean and **population variance**, `x` a new finite observation,
and `a=alpha`. Each eligible current-season observation at/after entry updates:
...
Missing/nonfinite observations are skipped without decay. At `a=1`, a finite
...
For seeded moments `(mE,vE)`, ordinary rolling population moments `(mR,vR)`, and
seeded-component weight `w`, blending uses:

```text
m = w*mE + (1-w)*mR
v = w*vE + (1-w)*vR + w*(1-w)*(mE-mR)^2
```

The between-means term is required. For `(2,4)`, `(10,16)`, `w=.25`, the result is
`m=8, v=25`. Endpoints return the selected pair without propagating missing values
from the unused pair. Weighted std is `sqrt(v)`. Warm Z uses these same warmed
...
| Policy | Weight `w` | Example |
| --- | --- | --- |
| `Hard(k)` | `1` if `r < k`, else `0` | `Hard(1)` gives `1,0,0` at `r=0,1,2`. |
| `LinearFade(start, rounds)` | `clip(1 - (r-start)/rounds, 0, 1)` | Defaults give `1,1,.5,0` at `r=0,1,2,3`. |
| `ObservationCount(strength)` | `strength/(strength+n)` | Strength 5 gives `1,.5,.25` at `n=0,5,15`. |
...
Public `rating = 1500 + 173.7178*mu`, `rd = 173.7178*phi`. Thus retained inflation
preserves `mu`, mover location is `(1-s)*mu_old + s*mu_prior`, and known movers use
`phi_new = phi_old*phi_scale*movement_phi_scale`. A mover without an existing
