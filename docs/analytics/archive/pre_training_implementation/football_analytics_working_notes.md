...
Loading, histories, features, ratings/transitions, labels, assembly and the corrected splits/CV API are implemented and verified. Pre-training analysis, the reusable reporter/viewer library and common feature selectors now pass the reporting checks below. Live Jupyter frontend/trust behavior remains unverified; saved-notebook HTML and iframe behavior are verified. Model adapters, post-training reports, fold-aware state preparation and general training orchestration remain before the corner experiment. Pilot recipe selection, tuning and fitting remain deferred. Historical results retain their original source provenance and archived context.
...
**Current implementation direction:** loading, observed histories, features, league/LOO, explicit warm starts, result/statistic rating transitions, labels and dataset assembly are verified. Splits/CV and pre-training analysis/reporting now pass independent checks. The common result/viewer and selector contracts are implemented. Fold-aware state preparation, model adapters, post-training reporters and generic training orchestration remain before the corner experiment. Existing league/season and eventual Lasso/corner choices remain context; pilot recipe selection, tuning and fitting are deferred. (D)

The generic skeleton connects persistent state preparation, feature and label evaluation, dataset assembly, temporal splitting, training-only fitted transformations/model adapters, and pre-training/post-training reporting. Feature evaluation should consume prepared rating states so reports, models and folds can reuse them. The orchestration must support multiple labels, models and prediction layouts without hard-coding corners, Lasso or one row format. Assembly, split and pre-training reporting APIs are now implemented; model execution, fold-aware state reconstruction and post-training APIs remain later work. (D)
