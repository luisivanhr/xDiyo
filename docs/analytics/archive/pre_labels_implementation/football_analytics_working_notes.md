...
The league/LOO, explicit warm-start and rating-transition batch is now implemented and verified. Next, return to the shared dataset contract and remaining reusable orchestration before configuring the corner experiment. The new APIs are opt-in; pilot recipe selection, tuning and fitting remain deferred. Historical design material remains below and in the preserved archive.
...
**Current implementation direction:** loading, observed histories, features, league/LOO, explicit warm starts and result/statistic rating transitions are verified. Return to the shared dataset contract, reusable labels, assembly, temporal splits, model adapters and both reporter stages before configuring the corner experiment. Existing league/season and eventual Lasso/corner choices remain context; pilot recipe selection, tuning and fitting are deferred. (D)
