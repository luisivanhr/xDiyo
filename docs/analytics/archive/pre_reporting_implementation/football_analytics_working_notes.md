...
The label and assembly batches are implemented and verified, following league/LOO, explicit warm starts and rating transitions. The corrected splits/CV API is implemented and verified, including focused runtime checks, refreshed documentation and the final preservation audit. Earlier full-suite and real-data results retain their original source provenance. Model adapters, reports, fold-aware state preparation and generic orchestration remain before the corner experiment. Pilot recipe selection, tuning and fitting remain deferred. Historical design material remains below and in the preserved archive.
