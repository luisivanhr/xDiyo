...
The label batch is implemented and verified, following league/LOO, explicit warm starts and rating transitions. Dataset assembly now supplies the reusable feature/label/metadata contract; all runtime, source and preservation checks pass. Temporal splits, model adapters, reports and generic orchestration remain before the corner experiment. Pilot recipe selection, tuning and fitting remain deferred. Historical design material remains below and in the preserved archive.
...
| **Configuration and orchestration** | Loading/features/state, labels and dataset assembly provide reusable components. Next design temporal splits, training-only transforms/model adapters, both report stages and their orchestration. Reuse prepared rating states. Exact orchestration interfaces remain open; pilot configuration, tuning and fitting are deferred. |
