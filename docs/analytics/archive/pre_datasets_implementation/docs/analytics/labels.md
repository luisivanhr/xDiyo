...
records verification. Dataset assembly and model fitting remain later work.
...
available for later filtering decisions by the dataset assembler. Match and
team-match outputs can coexist in this dictionary, but need an explicit alignment
and layout choice before joining them to a feature matrix. This increment adds
no assembler, train/test split, model fitting, odds, profit or reporter.
