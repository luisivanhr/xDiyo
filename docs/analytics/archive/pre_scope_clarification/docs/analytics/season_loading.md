...
Neither loader nor `inspect_season` requires a `coverage` table. It is not selected
for this experiment. Explicitly requesting it when absent raises a clear
`KeyError` naming the unavailable table. A clean future export can omit both the
`coverage` descriptor and `coverage.parquet` in its new version. Do not delete a
file behind an unchanged manifest or alter historical pinned versions. Other
tables remain usable and the catalogue lists only tables that are declared.
