...
For the next implementation session, use the verified population below and settle the first feature inputs and timing/history policies. The earlier wait-for-sample instruction and proposed loader plan are superseded by the completed import. The goals, artifacts, timing/CV decisions, and later template reference remain the broader context.
...
types, row order and nulls; the 57-test suite passed.
...
[nine-cell notebook](notebooks/01_loader_walkthrough.ipynb) shows practical usage
and match counts by league/season; [the guide](docs/analytics/season_loading.md)
explains selections and table access.

**Coverage is optional** and is not requested for this experiment. Both loaders
and the inspector work for exports with no coverage descriptor/file. A future
version may omit both together while old versions remain intact; do not remove
collected files behind unchanged manifests. An explicit request for absent
coverage raises `KeyError`. This behavior was tested using synthetic exports.
...
**Current implementation direction:** build the reusable system scaffold and initial feature library, using the Lasso experiment as a first test rather than hard-coding the library around it. Real source publications have now been inspected and the selected population imported through the verified analytics loader. Feature calculations, orchestration and reports remain subsequent work; the import does not settle feature inputs, historical availability or evaluation policy. (D)
