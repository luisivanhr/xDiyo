...
- [x] Preserved all 36 existing slim API tests unchanged and added 21 multi-season cases in `tests/analytics/test_multiseason.py`. **57 tests passed** with `.misc314 -m pytest tests/analytics -q -p no:cacheprovider`. Tests cover top-level discovery, exact filters/order, absent combinations, source values/nulls/optional columns, independent shots, unavailable tables, reserved origin columns, records/version pinning/new discovery, later failures, and opt-in hash forwarding.
- [x] Verified coverage omission in synthetic exports both from the outset and in a newly published version, preserving the old version. Both loaders, the one-table shortcut and inspector work without coverage; explicit absent-coverage requests raise `KeyError`.
...
**Checked source:** `loading.py` SHA256 `b77d29fbe2b09c4036475beb2138b779d8fcddb712e1c9222082c8497b784bbf`. All five package-source hashes were identical before/after the suite, real import and notebook execution. Full results and source/selection fingerprints: [multi-season import evidence](docs/analytics/multiseason_import_check.json).

**Coverage policy:** `coverage` is optional and is not selected for this experiment or required by either loader. A future publication can omit both its descriptor and file, and the catalogue follows the remaining manifest. Explicitly requesting absent coverage raises a clear unavailable-table `KeyError`. Do not delete collected files behind unchanged manifests or change historical pinned versions in place; the user's removal plan concerns future exports/library publication. Synthetic tests verify this policy without altering collected files.
