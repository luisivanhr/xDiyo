...
Rounds and seasons default to a separate calendar per `competition_id`.
Kickoff mode defaults to one pooled calendar, with simultaneous matches in the
same block. `calendar_by=()` explicitly pools calendars. A name or tuple of names
can select another grouping; `block_by` replaces the default block identity.
...
`model_selector=None` disables selection/search. `refit_policy=None` disables
additional scheduled refits; it does not mean omitting a future initial fit for
each fold. Today both options are merely stored, even when non-None objects are
passed. No initial fit, selector call or scheduled refit runs in this module.
