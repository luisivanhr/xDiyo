...
class SplitPlan:
    ...
    model_selector=None means no search/selection. refit_policy=None means no
    additional scheduled refits, not omission of the initial fit per fold.
    Non-None objects are retained for the future fitting layer, never executed
    by this module. paths maps CPCV (path_id, block_id) to fold_id and row positions.
    """
...
def create_split_plan(dataset, splitter, *, model_selector=None, refit_policy=None,
                      **split_options):
    """Prepare a split scheme, keeping selection/refitting explicitly opt-in.

    split_options passes row-aligned timing/group inputs to splitter.folds().
    This operation only builds membership and CPCV path mappings. It neither
    computes features nor invokes the supplied selector/refitting objects.
    """
