...
document.querySelectorAll('th button[data-sort]').forEach(button=>button.addEventListener('click',()=>{
  ...
  rows.sort((a,b)=>{
    ...
    if(ac.dataset.numeric==='true'&&bc.dataset.numeric==='true'){
      const av=Number(ac.dataset.value),bv=Number(bc.dataset.value);diff=absolute?Math.abs(av)-Math.abs(bv):av-bv;
...
def render_report(report, *, path=None, renderers=None):
    ...
    def artifact_html(artifact):
        ...
        if artifact.kind == "leaderboard":
            legend = '<p class="legend"><span class="pos">Positive</span> / <span class="neg">negative</span> coefficients · bar length = magnitude · click headers to sort</p>'
        ...
        if artifact.kind == "plotly":
            ...
            return f'<div class="plot" data-spec="{spec_id}"></div><script type="application/json" id="{spec_id}">{spec}</script>'
    ...
    for i, (name, runs) in enumerate(groups.items()):
        ...
        for j, run in enumerate(runs):
            ...
            downloads = []
            scope_table = pd.DataFrame({"row_position": run.row_positions})
            downloads.append(_csv_link(scope_table, f"{name}-scope-{j}"))
            for table_name, table in run.result.tables.items():
                downloads.append(f'<details class="artifact"><summary>Data: {_e(table_name)} ({len(table):,} rows)</summary>'
                                 + _csv_link(table, f"{name}-{table_name}-{j}") + _table(table) + '</details>')
            variants.append(f'<div class="variant" data-fold="{j}" {"hidden" if j else ""}>'
                            f'{scope}<h3>{_e(run.result.title)}</h3>{notes}{"".join(artifacts)}{"".join(downloads)}</div>')
...
<title>{_e(report.title)}</title><style>{CSS}</style>{plotly_script}</head><body>
