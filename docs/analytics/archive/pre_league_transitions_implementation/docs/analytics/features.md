...
`evaluate_features`. The result has the same row order and index as the history.
...
| `RollingZScore(source, window=5, min_periods=1, ddof=1)` | Latest eligible value minus the window mean, divided by its std. The latest value is included in the baseline. |
...
the ratings extension brings the current full analytics suite to **178 tests**.
