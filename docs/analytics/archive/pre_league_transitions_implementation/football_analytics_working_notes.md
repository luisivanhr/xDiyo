...
- **Season transitions — proposed shared helpers:** supply season-entry, movement and eligible-prior context for explicitly chosen policies; feature construction owns this step, independently of CV. Reuse the existing ID-based movement core with new-export inputs; a newly observed team is not automatically promoted. For Glicko, continuing-team uncertainty inflation could preserve `mu`; movement could blend `mu` toward a destination prior and optionally inflate `phi`, leaving `sigma` unchanged unless explicitly configured. These adjustments and their parameters remain optional and pending.
- **Policy selection — proposed interface:** attach an opt-in policy to each feature definition or named rating producer/stream, defaulting to `None` (no adjustment). Helpers provide transition context and never infer a rating type from columns. A rating stream's identity includes outcome/statistic, period, engine/configuration, scope and transition policy; prior values come from the same stream unless an explicit cross-stream mapping is configured. Warmed and unwarmed variants retain distinct states. Apply a stream's transition **once to its full state before extracting fields**, so separate `mu`/`phi` requests cannot repeat it. Custom engines need their own transition adapters; Glicko formulas must not be applied to embeddings. These interfaces are proposals, not implemented capabilities.
- **Rating priors and state transfer — rating-only cohort selection:** destination top-m/bottom-n cohorts ranked by standings (preferred default) or rating apply **only to rating warm-up priors, not ordinary features**. Freeze their snapshots before transition and exclude the mover; cohort sizes and mean/median remain configurable choices. Current ratings are competition-scoped, so retrieving previous-league state and calibrating/transferring league rating scales remain explicit operations; membership evidence alone does not settle them.
- **Mean warm starts and handoff — optional design:** ordinary feature expressions use statistic-specific team/league mean priors from eligible previous-season or destination-league history, with explicit blend, decay and time unit; they do not use rating top/bottom cohorts. The simple baseline is a **hard EMA-to-rolling handoff after 1, 2 or 3 completed rounds**, then the ordinary configured rolling window with available samples, no padding or automatic full-window requirement. **One round when enabled is a tentative recommendation, not a confirmed default:** it mainly supplies the opening-round prior, whereas at least two rounds let later predictions use the updated EMA. Gradual fading by rounds and blending by effective observation count are alternatives only; neither is selected for implementation. Continuing an EMA indefinitely is a separate policy. Global `warmup=None` remains the default; James-Stein and more elaborate borrowing remain deferred.
...
| **Season and league transitions** | No-warm-up/available-history remains the default. Shared helpers provide transition context for explicit optional policies. Reuse ID-based movement evidence with new exports; frozen destination top/bottom cohorts exclude the mover and apply only to rating priors, with standings (preferred default) or rating ranking. Ordinary feature priors are statistic-specific. Exact adjustments, state transfer and cross-league rating calibration remain open. Boundaries do not automatically reset models or CV. |
| **Warm-up policy selection** | Proposed: opt in per feature definition or named rating producer/stream, default `None`. Helpers never select a rating from column names. Stream identity includes outcome/statistic, period, engine/configuration, scope and transition policy; priors match the stream and warmed/unwarmed states stay separate. Apply the full-state transition once before extracting fields; custom engines need their own adapters. Exact interfaces remain unimplemented. |
| **Optional prior/exponential strategy** | `warmup=None` remains the default. Hard EMA-to-rolling handoff after 1/2/3 completed rounds is the simple baseline; one round when enabled remains tentative. Round-based fading and effective-observation-count blending are unselected alternatives. Variability uses matching prior-season league data (destination league for movers), with consistent mean/spread updates and no silent zero for missing variance. Weighting, sample correction and fallback remain open. `LinearWarmStart` remains illustrative; James-Stein is deferred. Glicko retains its own update rules. |
