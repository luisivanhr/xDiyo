...
The label batch is now implemented and verified, following league/LOO, explicit warm starts and rating transitions. Next, align the reusable feature/label/metadata contract in the dataset assembler before configuring the corner experiment. Pilot recipe selection, tuning and fitting remain deferred. Historical design material remains below and in the preserved archive.
...
**Current implementation direction:** loading, observed histories, features, league/LOO, explicit warm starts, result/statistic rating transitions and reusable labels are verified. Next, integrate their shared dataset contract in assembly, then address temporal splits, model adapters and both reporter stages before configuring the corner experiment. Existing league/season and eventual Lasso/corner choices remain context; pilot recipe selection, tuning and fitting are deferred. (D)
...
| --- | --- |
| **Reusable library and experiment interface** | League/transition APIs are verified. Next define the shared X/y/metadata/prediction-unit contract, initially match/team-match with group identities, then reusable orchestration before pilot configuration. Exact assembly/model/report interfaces, GUI and distribution remain open. |
...
| **Historical grouping and alignment** | Team grouping and H2H remain supported for ordinary histories. League team/match units, all three window units, both schedules, pooled moments and no-refill contribution/fixture LOO are now verified. League H2H is unsupported. Match-wide assembly remains later work. |
...
| **Configuration and orchestration** | Next design the reusable skeleton over verified loading/features/state: dataset assembly, labels, temporal splits, training-only transforms/model adapters and both report stages. Reuse prepared rating states. Exact orchestration interfaces remain open; pilot configuration, tuning and fitting are deferred. |
