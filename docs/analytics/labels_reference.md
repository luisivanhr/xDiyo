...
The dictionary can mix match and team-match targets. Select one `LabelData` for
[assemble_dataset](datasets_reference.md), with an explicit matching layout,
optional target selection and missing-target filtering. It aligns feature rows
by identity and preserves paired match groups. These labels do not choose prediction times, train/test
