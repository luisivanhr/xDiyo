...
## Select statistics after loading

Use `select_stats(experiment, bundles=["attack_totals", "defense_first_half", "standings"],
stats=[("ALL", "Match overview", "ballPossession")])` to combine bundles and exact
statistics. The [selection guide](stat_selection.md) defines categories, period
filters and per-call overrides. Matches, selected statistics and pregame positions
stay separate; shots remain independently selectable.
