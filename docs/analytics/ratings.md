...
| `mu` | Internal coordinate \(\mu=(\mathrm{rating}-1500)/173.7178\). |
| `phi` | Internal deviation \(\phi=\mathrm{RD}/173.7178\). |
...
reset independently by season when no transition policy is supplied. Explicit
transitions may transfer predecessor state into the new scoped key. Rating nodes own this scope; the evaluator's
...
Generated expressions cache each source/engine/scope/direction/transition-policy stream within
...
## Optional season and movement transitions

`build_ratings(..., transition=GlickoTransition(...), team_seasons=...,
season_starts=...)` prepares transitioned states. Generated expressions opt in
with `WarmStart(MatchResultGlicko(...), policy)` or `WarmStart(StatGlicko(...), policy)`.
The transition acts once on full state before field extraction and equal-time
result updates. Warmed/unwarmed streams remain separate; no wrapper/policy keeps
the existing replay. Saved `Rating(name)` runs must be transitioned during building.

Ranked top/bottom destination cohorts apply only to rating priors. They use frozen
prior-season members, exclude incoming teams and may include departing members.
Standings ranking uses latest eligible pregame positions, not a final table.
Custom engines require their own transition adapter. See the
[practical transition guide](league_warmup.md#rating-transitions-and-reuse),
[complete API/extension reference](league_warmup_reference.md#glickotransition),
[coverage checklist](league_warmup_documentation_checklist.md), and separate
[notebook](../../notebooks/04_league_warmup_quickstart.ipynb).
...
The original rating batch passed **37 focused tests** and **178 full-suite tests**.
The league/transition extension now passes **275 tests**, including 97 new cases;
see [its independent evidence](league_warmup_check.json).
