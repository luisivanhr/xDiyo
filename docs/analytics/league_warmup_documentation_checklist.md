...
### Equation follow-up — 17 September 2026

The documentation-only follow-up was reviewed against the same 23 library/helper
source-file hashes as the implementation verification. The concurrent addition
of the labels package to `pyproject.toml` is outside this equation review and is
being verified separately. All 77 LaTeX expressions across the
practical and ratings guides parsed and rendered without errors after repairing
three unescaped parameter-name underscores. Seven rendered sections were inspected;
display equations and table cells fit the review page. The finite-observation set,
warm-up fallback and selected-cohort notation were made explicit. User-facing
equations remain LaTeX; API signatures and Python examples retain code formatting.

| Equation group | Reviewed explanation |
| --- | --- |
| Finite population, pooled mean, variance and standard deviation | [Pooled moments](league_warmup.md#loo-and-pooled-moments) |
| Z-score, positive spread and ordinary versus warmed moments | [Z-score references](league_warmup.md#cutoffs-and-z-score-references) |
| Team/mover/league mean prior and league population variance | [Frozen priors](league_warmup.md#frozen-priors-and-missing-data) |
| Seeded EMA, missing observations and complete replacement | [EMA equations](league_warmup.md#ema-and-mixture-equations) |
| First/second-moment mixture, between-means variance and ddof conversion | [Mixture equations](league_warmup.md#ema-and-mixture-equations) |
| Hard, linear and observation-count handoff weights | [All handoffs](league_warmup.md#all-three-handoffs) |
| Public/internal rating and deviation coordinates | [Rating scales](ratings.md#one-numerical-engine-explicit-scales) |
| Selected-cohort mean/median, location shrinkage, deviation inflation and unchanged volatility | [Glicko transitions](league_warmup.md#glicko-policies-and-rating-only-cohorts) |

Bounded symbolic and numerical equation checks passed. The 18 previously executed
league/reference examples retain their recorded code hashes; ratings examples and
all notebooks are also unchanged. The full implementation suite was not rerun.
Separate evidence: [equation_documentation_check.json](equation_documentation_check.json).
The original implementation evidence below remains unchanged.
