...
def _event_membership(values, candidates):
    """Compare exact IDs without Arrow inferring a signed membership array."""
    import pandas as pd

    candidates = set(candidates)
    return pd.Series([False if pd.isna(event) else event in candidates for event in values.tolist()],
                     index=values.index, dtype=bool)
...
def _check_statistics(frame, matches=None):
    ...
    if not _event_membership(frame.event_id, teams).all():
...
def load_season(data_root, season_stem, *, tables=("matches",), record_path=None,
                verify_hashes=False, include_awarded=False) -> SeasonData:
    ...
    include_awarded=False excludes rows explicitly flagged is_awarded=True and
    ...
    excluded = []
    matches = frames.get("matches")
    if not include_awarded:
        if matches is None and any("event_id" in frame and not frame.empty for frame in frames.values()):
            matches = _read_table(source, "matches", verify_hashes)
            _check_matches(matches, source.manifest["scope"])
        if matches is not None and "is_awarded" in matches:
            excluded = matches.loc[matches.is_awarded.eq(True).fillna(False), "event_id"].tolist()
            if excluded:
                frames = {name: frame.loc[~_event_membership(frame.event_id, excluded)].reset_index(drop=True)
                          if "event_id" in frame else frame for name, frame in frames.items()}
...
def load_season_table(data_root, season_stem, *, table="matches", record_path=None,
                    verify_hashes=False, include_awarded=False):
...
def load_seasons(data_root, seasons, *, leagues=None, tables=("matches",),
    ...
    provenance = {"seasons": tuple(seasons), "leagues": tuple(sorted({l for _, l, _ in selected})),
                  "tables": tuple(tables), "sources": sources, "include_awarded": include_awarded}
