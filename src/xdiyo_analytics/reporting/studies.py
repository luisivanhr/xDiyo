...
class FeatureDistributionReporter:
    """Density histograms and Gaussian KDE for selected features and labels.
    ...
    features=None includes all available inputs; targets=None omits labels.
    Set targets to label column names to add their distributions over the same
    context rows. Label plots are prefixed 'Label' and label tables are separate
    (label_summary, label_histogram, label_kde), preserving feature table schemas.
    """
    ...
    def run(self, context):
        ...
        features = [] if not len(context.X.columns) and self.features is None else _columns(context.X, self.features, "features")
        targets = [] if self.targets is None else _columns(context.y, self.targets, "targets")
        ...
        result = StudyResult("Feature and label distributions" if targets else "Feature distributions", notes=[
        ...
        selected = [("feature", context.X, name) for name in features] + [("label", context.y, name) for name in targets]
        for kind, frame, feature in selected:
            raw = _numeric(frame[feature])
            ...
            if len(values):
                ...
                histogram.extend(dict(kind=kind, feature=feature, left=left, right=right, count=int(count), density=float(d))
                ...
                if status == "ok":
                    try:
                        ...
                        curves.extend(dict(kind=kind, feature=feature, x=float(x), density=float(d)) for x, d in zip(grid, kde))
            ...
            summary.append(dict(kind=kind, feature=feature, rows=len(raw), finite=len(values),
                                excluded=len(raw) - len(values), kde_status=status))
            title = f"Label · {feature}" if kind == "label" else str(feature)
            result.artifacts.append(Artifact("plotly", _style(figure, title, str(feature), "Density"), title))
        tables = {"summary": pd.DataFrame(summary),
                  "histogram": pd.DataFrame(histogram, columns=["kind", "feature", "left", "right", "count", "density"]),
                  "kde": pd.DataFrame(curves, columns=["kind", "feature", "x", "density"])}
        result.tables = {name: table.loc[table.kind == 'feature'].drop(columns='kind').reset_index(drop=True)
                         for name, table in tables.items()}
        if targets:
            result.tables.update({f'label_{name}': table.loc[table.kind == 'label'].drop(columns='kind')
                                  .rename(columns={'feature': 'label'}).reset_index(drop=True)
                                  for name, table in tables.items()})
        return result
        ...
        for feature in _columns(context.X, self.features, "features"):
            ...
            if self.entity == "team":
                ...
                if self.teams is not None:
                    ...
                    selected_teams = {team_key(team) for team in self.teams}
                    points = points[points.team_id.map(team_key).isin(selected_teams)].copy()
