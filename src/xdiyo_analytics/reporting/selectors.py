...
def resolve_selection_count(k, n_features, *, name="k"):
    """Resolve a count or a proportion of the supplied input feature population.

    Values strictly between zero and one request ceil(k * n_features).
    Whole numbers request that many features (1 always means one feature).
    """
    if isinstance(k, (bool, np.bool_)) or not isinstance(k, Real) or not np.isfinite(k) or k <= 0:
        raise ValueError(f"{name} must be a positive integer or a proportion between 0 and 1.")
    if k < 1:
        return ceil(Decimal(str(k)) * n_features)
    if k != int(k):
        raise ValueError(f"{name} must be a positive integer or a proportion between 0 and 1.")
    return int(k)
...
def vote_selections(fold_results, *, feature_order, k, score_column=None):
    ...
    a count, or a proportion of feature_order (rounded upward).
    ...
    k = None if k is None else resolve_selection_count(k, len(order))
