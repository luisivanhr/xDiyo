...
verification of the recorded snapshot; the later MatchResult/viewer tree is
covered separately below. Live Jupyter frontend behavior remains unverified.
...
## Run independent folds and choose a capable model device

`ExecutionPolicy(n_jobs=1, device="cpu", inner_threads=1, gpu_jobs=1)` configures
one fold per job. CPU fold processes may overlap; candidate/outer-fold loops stay
sequential. Result order follows the requested folds. `execution=None` preserves
the previous serial path and model settings. Parallel observer events are replayed
in the parent when the fold completes; serial events remain live.

`DeviceAdapter` calls an explicit device-discovery/configuration contract. Native
adapters own model placement, tensor conversion and optional framework threads.
Plain sklearn estimators stay CPU. auto/CUDA workers are capped by gpu_jobs,
including auto fallback to CPU. A single fit/refit is not split across devices.

The frozen revision passes **1,650 tests (61 new)**, three guide examples, new
notebook 16, 20 offline browser checks and isolated wheel process/recovery use.
One tiny synthetic Torch adapter was verified on cuda:0, including parameters and
input tensors. Fifteen prior notebooks and 3,295 earlier artifacts are unchanged.

**Known runtime limitation:** notebook computation passes, but kernel shutdown
emits loky `resource_tracker` KeyErrors on the verified Python 3.14/joblib 1.5.2
environment. Plain-joblib controls reproduce this even with max_nbytes=None.
Clean shutdown is not claimed; no package or source workaround was applied.
Live Jupyter frontend, concurrent/multiple-GPU execution and newer UI source are
outside this verification scope. All fits use synthetic inputs.

- [Execution guide](../docs/analytics/execution_policy.md)
- [API reference](../docs/analytics/execution_policy_reference.md)
- [Coverage](../docs/analytics/execution_policy_documentation_checklist.md)
- [Notebook 16](../notebooks/16_execution_policy_quickstart.ipynb)
- [Frozen verification and runtime logs](../docs/analytics/execution_policy_check.json)

## Select prediction fixtures and reopen a fitted model

`load_prediction_fixtures` reads ordinary `data/xDiyo_data` publications while
keeping completed and future history in `.data`. Choose published integer/list/
per-league rounds, build features on full history, assemble with
`drop_missing_targets=False`, and align the chosen fixture rows exactly.
Analytics loaders exclude explicit awarded matches and linked rows by default;
`include_awarded=True` retains them. Unknown flags remain included.

`save_model`/`load_model` and `FittedModel.save` preserve a fitted estimator and
its preprocessing for future prediction. Custom adapters supply native serializers.
Model directories must be new. Load joblib artifacts only from trusted producers
in compatible environments. Numerical experiment recovery still returns
`model=None`; saving a fitted model does not establish mid-fit resumability.

The preserved revision passes **1,589 analytics tests (79 new)**, four guide
examples, notebook 15, 19 offline browser checks and isolated wheel use.
Fourteen earlier notebooks and 2,903 earlier artifacts are unchanged. All fits
are synthetic; live Jupyter frontend remains unverified. The newer parallel/
device-policy source has a separate verification scope.

- [Prediction/persistence guide](../docs/analytics/prediction_persistence.md)
- [API reference](../docs/analytics/prediction_persistence_reference.md)
- [Coverage](../docs/analytics/prediction_persistence_documentation_checklist.md)
- [Notebook 15](../notebooks/15_prediction_persistence_quickstart.ipynb)
- [Verified source and evidence](../docs/analytics/prediction_persistence_check.json)

## Run, reuse and reopen an experiment

`FootballExperiment` combines explicit preparation, fixed or selected models,
reports and numerical persistence. Exact repeated runs reuse completed results;
interrupted searches reuse completed trials. Capable adapters can additionally
resume native fit checkpoints. `load(run_id)` skips preparation/fitting/prediction,
restoring report and prediction evidence with model objects set to None.
`RefitPolicy` requests an additional fresh deployment fit on explicit rows.

The preserved revision passes **1,510 analytics tests (68 new)**, five guide
examples, notebook 14, 26 offline browser checks and isolated wheel use.
Thirteen prior notebooks and 2,432 earlier artifacts are unchanged. Fits are
synthetic; live Jupyter frontend is unverified. Newer fixture-selection and
explicit model-persistence work has a separate verification record.

- [Practical experiment guide](../docs/analytics/football_experiment.md)
- [API reference](../docs/analytics/football_experiment_reference.md)
- [Recovery invariants](../docs/analytics/football_experiment_equations.md)
- [Coverage](../docs/analytics/football_experiment_documentation_checklist.md)
- [Notebook 14](../notebooks/14_football_experiment_quickstart.ipynb)
- [Verified source and evidence](../docs/analytics/football_experiment_check.json)

## Select a configuration and evaluate it independently

`ModelSelection` optionally compares finite `Candidate` lists or
`GridCandidates` within explicitly declared development rows. Every candidate
constructs fresh model and preprocessing state. Train-scoped feature selection
is rerun for each fitting population. Scalar, weighted and practical-tolerance
parsimony rules reuse numerical evidence; fitted AIC/BIC require explicit
model-provided likelihood statistics and conventions.

`SelectionResult.evaluate` fits the selected configuration freshly and returns
predictions on an untouched outer fold. `run_nested` chooses a separate winner
for every outer fold. Trial records remain separate from an explicitly saved
final evaluation; the default leaderboard shows final records.

The verified revision passes **1,442 analytics tests (130 new)**, seven examples,
notebook 13, 37 offline browser checks and isolated wheel use. Twelve prior
notebooks and 2,037 earlier artifacts remain unchanged. All fits are synthetic.
Live Jupyter frontend behavior remains unverified. Adaptive search,
ExperimentRunner and scheduled retraining remain later work.

- [Practical selection guide](../docs/analytics/model_selection.md)
- [All options, callbacks and helpers](../docs/analytics/model_selection_reference.md)
- [Selection equations](../docs/analytics/model_selection_equations.md)
- [Coverage checklist](../docs/analytics/model_selection_documentation_checklist.md)
- [Notebook 13](../notebooks/13_model_selection_quickstart.ipynb)
- [Verification evidence](../docs/analytics/model_selection_check.json)

## Inspect individual fixtures

`MatchResultReporter` uses retained predictions to show observed results, predictions
and numeric errors beside stable team names. Team-match layouts place separate
home and away label groups on each fixture row. Initial filters preserve the full
table; filtered CSV includes all matching observations across pages. Colors use
retained status, while the compact view shows Result/Prediction and optional
Error/Probabilities. Probability decisions require an explicit rule.

`TeamCatalog` supplies ID-based names and optional local badge paths. The optional
repository catalog contains 289 teams and 286 badges; saved reports embed accepted
images for offline use. The catalog is separate from the wheel.

The frozen revision passes **1,312 analytics tests (96 new)**, six examples,
notebook 12, 48 fixture and 28 saved-notebook/guide browser checks. Shared controls
also pass 26 browser regression checks. All eleven prior notebooks and 1,601
earlier artifacts remain unchanged. No real model fitting or performance claim
is included. Live Jupyter frontend behavior remains unverified.

- [Practical match-result guide](../docs/analytics/match_results.md)
- [All options and helpers](../docs/analytics/match_results_reference.md)
- [Comparison and display equations](../docs/analytics/match_results_equations.md)
- [Coverage checklist](../docs/analytics/match_results_documentation_checklist.md)
- [Notebook 12](../notebooks/12_match_results_quickstart.ipynb)
- [Verification evidence](../docs/analytics/match_results_check.json)
...
layers include fold-aware persistent-state preparation and adaptive search
