...
class TeamGAT(nn.Module):
    def __init__(self, num_teams, input_dim, hidden_dim, num_heads=2, num_layers=2, dropout=0.1, pad_idx=0):
        super().__init__()
        
        # Initial team embeddings (could use pretrained or learned embeddings)
        self.embedding = nn.Embedding(num_teams, input_dim, padding_idx=pad_idx)
        # GAT layers
    ...
    def forward(self, edge_index):
        ...
        edge_index = edge_index.to(self.embedding.weight.device)
...
class TeamGNN(nn.Module):
    def __init__(self, num_teams, input_dim, hidden_dim, num_layers=2, device='cpu', pad_idx=0):
        """
        Args:
            num_teams: Total number of teams.
            input_dim: Dimensionality of the initial team embeddings.
            hidden_dim: Hidden dimensionality for GNN layers.
            num_layers: Number of GCN layers.
        """
        super().__init__()
        # Initial team embeddings (you can initialize this as you do with nn.Embedding)
        self.embedding = nn.Embedding(num_teams, input_dim, padding_idx=pad_idx)
        self.device = device
        # Define a sequence of GCN layers.
        ...
        # Optionally, map back to the original embedding dimension.
    ...
    def forward(self, edge_index):
        ...
        Returns:
            Updated team embeddings of shape (num_teams, input_dim)
        """
        edge_index = edge_index.to(self.device)
        x = self.embedding.weight  # shape: (num_teams, input_dim)
...
class TeamEmbedding(nn.Module):
    def __init__(self, embed_dim, num_teams, mu_sigma, pad_idx=0, stats_hidden_dim=None, 
        ...
        # Embedding layer
        # Standard team embeddings are given by a GNN module.
        self.team_gnn = TeamGAT(num_teams=num_teams, input_dim=embed_dim, hidden_dim=embed_dim, num_heads=2,
                                num_layers = gnn_layers, dropout=0.1, pad_idx=pad_idx)
    ...
    def forward(self, batch,edge_index, key=None):
        ...
        # Update team embeddings using the GNN.
        updated_team_embeds = self.team_gnn(edge_index)  # shape: (num_teams, embed_dim)
        ...
        if self.training_embeds:
            ...
            home_team_embed = updated_team_embeds[home_team_indexes]
            away_team_embed = updated_team_embeds[away_team_indexes]
            ...
            team_embed = updated_team_embeds[team_indexes]
            enemy_embed = updated_team_embeds[enemy_indexes]
