...
def get_heatmaps(league, path, waiting_time):
    """
...
def get_recent_heatmaps(league, path, waiting_time, num_previous_matches):
    """
...
def get_lineups(league,path,waiting_time):
    options = webdriver.ChromeOptions()
...
def get_recent_lineups(league,path,waiting_time,num_previous_matches):
    options = webdriver.ChromeOptions()
    options.set_capability(
        'goog:loggingPrefs', {'performance': 'ALL', 'browser': 'ALL'}
    )
...
def force_get_lineups(lineups_df,path,waiting_time):
...
def scrape_data(league_id,season_id,num_rounds,num_teams,path,waiting_time,with_xG,with_comments):
    options = webdriver.ChromeOptions()
    options.set_capability(
        'goog:loggingPrefs', {'performance': 'ALL', 'browser': 'ALL'}
    )
    ...
    for i in range(1,num_rounds+1):
        ...
        for id in match_ids:
            ...
            match_stats={}
            if stats_data!={"error":{"code":404,"message":"Not Found"}} and 'statistics' in stats_data:
...
def scrape_recent_data(league_id,season_id,round,num_teams,path,waiting_time,with_xG):
    options = webdriver.ChromeOptions()
    options.set_capability(
        'goog:loggingPrefs', {'performance': 'ALL', 'browser': 'ALL'}
    )
...
def scrape_league_data(league_id, season_id, num_rounds, num_teams,
                    league_name, season_start, season_end, path, waiting_time=10,
    ...
    matches, standings, stats, shots, comments = scrape_data(league_id, season_id, num_rounds,
                                                            num_teams, path, waiting_time, with_xG, with_comments)
    league = get_league(matches, standings, stats, shots)
    
    # Add heatmap data if required
    if with_heatmaps:
        heatmaps = get_heatmaps(league, path, waiting_time)
    ...
    if with_lineups:
        lineups = get_lineups(league, path, waiting_time)
        lineup_df = pd.DataFrame.from_dict(lineups, orient='index')
        lineup_df = force_get_lineups(lineup_df, path, waiting_time)
...
def scrape_league_recent(league_id, season_id, num_rounds, num_teams,
    ...
    matches, standings, stats, shots, num_previous_matches = scrape_recent_data(
        league_id, season_id, num_rounds, num_teams, path, waiting_time, with_xG
    )
    ...
    if with_lineups:
        lineups = get_recent_lineups(league, path, waiting_time, num_previous_matches)
        lineup_df = pd.DataFrame.from_dict(lineups, orient='index')
