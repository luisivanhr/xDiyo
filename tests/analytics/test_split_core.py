...
import pandas as pd
import pytest
from xdiyo_analytics.splits import Fold, SplitPlan, GroupKFold, MatchKFold, create_split_plan
...
def test_split_plan_owns_membership_only_and_supports_independent_reporting():
    class MustNotRun:
    ...
    assert set(SplitPlan.__dataclass_fields__) == {'folds','n_rows','row_order','paths'}
    assert list(inspect.signature(create_split_plan).parameters) == ['dataset','splitter','split_options']
    assert plan.paths is None
    for name in ['model_selector','refit_policy']:
        assert not hasattr(plan,name)
        with pytest.raises(TypeError): create_split_plan(data,splitter,**{name:MustNotRun()})
        with pytest.raises(TypeError): SplitPlan(plan.folds,plan.n_rows,plan.row_order,**{name:None})
    report = pd.DataFrame([{'train':len(f.train),'test':len(f.test),'score':len(f.score)} for f in plan.folds])
    assert report.to_dict('list') == {'train':[8,8,8],'test':[4,4,4],'score':[4,4,4]}
